...
Partial Class modulo_cursos_reporte_bitacoras
    ...
    Dim objCursoInt As New CCursoInterno
    ...
    Public Sub Consultar()
        Try
            ...
            'cabecera curso interno
            If ViewState("Tipo") = 0 Then
                objCursoInt.Inicializar0(objSql, objSession.Rut)
                objCursoInt.Inicializar1(ViewState("CodCurso"), objSession.Agno)
                Me.lblCorrelativoInterno.Text = objCursoInt.Correlativo
                Me.lblCorrelativoEmpresaInterno.Text = objCursoInt.CorrEmpresa
                If objCursoInt.CodEstadoCurso = 1 Then
                    Me.lblEstadoInterno.Text = "Ingresado"
                ElseIf objCursoInt.CodEstadoCurso = 2 Then
                    Me.lblEstadoInterno.Text = "Anulado"
                End If
                Me.lblFechaInterno.Text = objCursoInt.FechaInicio
                Me.tbBitacoraCursoInterno.Visible = True

                objBitacora = New CReporteBitacora
                objBitacora.FechaInicio = calFechaInicial.SelectedValue
                objBitacora.FechaTermino = calFechaFinal.SelectedValue
                objBitacora.TipoReferencia = 6 'curso interno
                objBitacora.RutUsuario = 0 'Esto no es un error, consulta por todos los ruts
                objBitacora.CodCurso = lngCodCurso
                Dim dt As New DataTable
                dt = objBitacora.Consultar()
        ...
        Catch ex As Exception
            EnviaError("reporte_bitacoras:Consultar-->" & ex.Message)
        End Try
